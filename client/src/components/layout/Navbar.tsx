...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Check if we should update scrolled state - used for main navbar
      if (Math.abs(currentScrollY - lastScrollY.current) > 5) {
        ...
        // Update contact bar visibility separately
        setShowContactBar(currentScrollY <= 50);
  ...
  return (
    <header className="sticky top-0 z-50 w-full flex flex-col">
      {/* Contact information bar - with absolute positioning to prevent layout shift */}
      <div 
        className={`bg-[hsl(var(--vitality-green))] transition-all duration-300 absolute top-0 left-0 w-full z-20 ${
          showContactBar ? "h-[50px] opacity-100 visible" : "h-0 opacity-0 invisible"
        }`}
      >
      ...
      {/* Main navbar - with margin-top that adjusts based on contact bar visibility */}
      <div 
        className={`transition-all duration-300 bg-white w-full ${
          scrolled ? "py-3" : "py-5"
        } ${showContactBar ? "mt-[50px]" : "mt-0"}`}
      >
